...
class ScalableGNN(torch.nn.Module):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        out = self.forward(g, x, batch_size, n_id, offset, count, **kwargs)
    ...
    def mini_inference_serial(self, loader: SubgraphLoader) -> Tensor:
        ...
        for subgraph, batch_size, n_id, offset, count, state in loader:
            subgraph = subgraph.to(self.device)
            x = subgraph.srcdata['feat']
            out = self.forward_layer(0, subgraph, x, state)[:batch_size]
        ...
        for i in range(1, len(self.histories)):
            ...
            for subgraph, batch_size, n_id, offset, count, state in loader:
                ...
                out = self.forward_layer(i, subgraph, x, state)[:batch_size]

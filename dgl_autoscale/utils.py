...
def compute_auc(logits: Tensor, y: Tensor,
...
def dropout(graph: DGLBlock, transform_dropedge, training: bool = True):
    if not training or transform_dropedge.p == 0.:
        return graph

    if 'weight' in graph.edata:
        if 'weight_clone' not in graph.edata:
            graph.edata['weight_clone'] = graph.edata['weight']
        value = F.dropout(graph.edata['weight_clone'], p=transform_dropedge.p)
        graph.edata['weight'] = value
